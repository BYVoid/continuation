...
CallExpression.prototype.normalize = function (place) {
  ...
  for (var i = 0; i < this.arguments.length; i++) {
    this.arguments[i].normalize(place);
  }
...
CallExpression.prototype.transformParallel = function (place) {
  ...
  var expPlaces = [];
  this.arguments.forEach(function (expression) {
    var expPlace = expression.transform();
  ...
  var innerPlace = [];
  var nextPlace = [];
  var parallelCounter = new Identifier('_$parallel_done');
  var cont = new Identifier(helpers.continuationIdentifier);
  var errId = new Identifier(helpers.errName);
  var errArray = new Identifier('_$errors');
  ...
  //function (_$err)
  this.arguments = [new FunctionExpression(
    null,
    [errId],
