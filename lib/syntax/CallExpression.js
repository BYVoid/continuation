...
CallExpression.prototype.normalize = function (aliases) {
  var self = this;
  
  findContinuation(this.arguments, aliases, function (contExpr, contPos, options) {
    if (options.obtain) {
...
CallExpression.prototype.transform = function (place, aliases) {
  var self = this;
  
  place = this.callee.transform(place, aliases);
  
  if (this.callee.type === 'Identifier' && this.callee.name === aliases.parallelName) {
    return this.transformParallel(place, aliases);
  }
  
  this.arguments.forEach(function (expression) {
    place = expression.transform(place, aliases);
  });
...
var findContinuation = function (args, aliases, callback) {
  //Todo check multiple cont && obtain
  for (var i = 0; i < args.length; i++) {
    var arg = args[i];
    if (arg.type === 'CallExpression') {
      if (arg.callee.name === aliases.contName) {
        return callback(arg, i, {});
      } else if (arg.callee.name === aliases.obtainName) {
...
CallExpression.prototype.transformParallel = function (place, aliases) {
  ...
  var expPlaces = [];
  this.arguments.forEach(function (expression) {
    var expPlace = expression.transform(place, aliases);
